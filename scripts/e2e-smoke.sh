...
"$repo_root/Forge" --version >/dev/null
...
"$repo_root/tinypm" bundle list >/dev/null
"$repo_root/tinypm" history 10 >/dev/null
"$repo_root/tinypm" pinned >/dev/null
"$repo_root/version" >/dev/null

version_output="$(mktemp)"
TINYPM_FLAVOR=abora "$repo_root/version" >"$version_output"
grep -q 'Abora TinyPM V4 / Forge v4.0.0' "$version_output"
rm -f "$version_output"
...
"$HOME/.local/bin/Forge" --version >/dev/null
...
"$HOME/.local/bin/tinypm" bundle list >/dev/null
"$HOME/.local/bin/tinypm" pinned >/dev/null
...
grep -q 'Abora TinyPM V4 / Forge v4.0.0' "$installed_version_output"
